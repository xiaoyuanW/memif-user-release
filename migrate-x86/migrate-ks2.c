...
static int dma_complete_one_mig(struct mig_pg_desc *desc)
{
	...
	if (!__sync_bool_compare_and_swap(desc->ptep, desc->pte, pte)) {
		 ...
		 */
		E("Race detected.");
//		BUG();  /* this will lead kernel to lockup, why? */
		return -1;
	}
#endif
		...
		mig_desc *desc, int do_irq, struct mm_struct *mm)
{
	...
	/* DMA. Thanks to IO coherency, no map/unmap needed */
	tx = info->chan->device->device_prep_dma_sg(info->chan,
			sg_dst, count, sg_src, count, tx_flags);
		...
		mig_desc *desc, int do_irq, struct mm_struct *mm)
{
	...
	if (do_irq) {
		tx->callback = dma_callback_kthread;
//		tx->callback = dma_callback_dummy;
		tx->callback_param = (void *)table;
	}
